...
public plugin_init() {
	...
	holyid = zp_register_extra_item("Holy Hand Grenade", 20, ZP_TEAM_HUMAN);
...
public kill(const Float:originF[3], attacker){
	...
	while ((victim = engfunc(EngFunc_FindEntityInSphere, victim, originF, radius)) != 0)
	{
		...
		ExecuteHam(Ham_TakeDamage, victim, 0, attacker, get_pcvar_float(cvar_holydmg), DMG_BLAST|DMG_NEVERGIB);
