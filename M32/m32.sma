...
public plugin_init() {
	...
	cvar_damage = register_cvar("m32_damage", "75.0");
	cvar_ammo = register_cvar("m32_ammo", "6");
	cvar_icon = register_cvar("m32_kill_icon", "0");
}
		...
		if(is_user_alive(i)){
			...
			if(distance <= 100){
				if(get_user_team(id) != get_user_team(i)){
					...
					ExecuteHamB(Ham_TakeDamage, i, 0, id, get_pcvar_float(cvar_damage), DMG_BLAST);
					set_msg_block(g_secdeath, BLOCK_NOT);
					if(get_user_health(i) <= 0){
						if(get_pcvar_num(cvar_icon) == 0){
							SendDeathMsg(id, i, 0, "m3");
						}
						else if(get_pcvar_num(cvar_icon) == 1){
							SendDeathMsg(id, i, 0, "grenade");
						}
					}
				}
			}
...
public fw_ReloadWeapon_2(weapon_entity){
	...
	if(g_hasm32[id] && clip < get_pcvar_num(cvar_ammo) && bpammo > 0){
		if(!task_exists(1000)){
			new data[1]
			data[0] = id
...
public reload(data[]){
	...
	if(clip > get_pcvar_num(cvar_ammo) - 1 || bpammo < 1){
		set_weapon_animation(id, 4);
		g_reload[id] = false
