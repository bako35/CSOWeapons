...
public plugin_init() {
	...
	m3dragon = zp_register_extra_item("M3 Dragon", 0, ZP_TEAM_HUMAN);
}
...
public dragon_damage(data[]){
	...
	if(pev_valid(weapon_entity)){
		ExecuteHamB(Ham_TakeDamage, vicid, weapon_entity, attid, 2.0, DMG_BURN|DMG_NEVERGIB);
	}
	else{
		ExecuteHamB(Ham_TakeDamage, vicid, 0, attid, 2.0, DMG_BURN|DMG_NEVERGIB);
	}
}
