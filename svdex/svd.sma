...
public plugin_init() {
	register_plugin("SVD", "1.0", "bako35");
	register_clcmd("gun", "give_svd");
	register_clcmd("say /svdammo", "give_svdammo");
...
public give_svdammo(id){
	if(is_user_alive(id) && g_hassvd[id] && ammox[id] < get_pcvar_num(cvar_alt_ammo)){
		ammox[id] = get_pcvar_num(cvar_alt_ammo)
		set_sec_ammo(id, ammox[id]);
		cs_set_user_bpammo(id, CSW_AK47, 200);
		...
		if(is_user_alive(i)){
			...
			if(distance <= 100){
				if(get_user_team(id) != get_user_team(i)){
					...
					ExecuteHamB(Ham_TakeDamage, i, 0, id, get_pcvar_float(cvar_alt_damage), DMG_BLAST);
