...
public plugin_init() {
	...
	register_clcmd("gun", "give_gungnir");
...
public plugin_precache(){
	...
	precache_generic("sprites/640hud185.spr");
	precache_generic("sprites/bakoweapon_gungnir.txt");
...
public PrimaryAttack(id){
	...
	for(k = 0; k < 3; k++){
		while((pEntity = engfunc(EngFunc_FindEntityInSphere, pEntity, fOrigin, 256.0)) != 0){
			...
			if(is_user_alive(g_ivic[k]) && entity_range(id, g_ivic[k]) < 256.0){
