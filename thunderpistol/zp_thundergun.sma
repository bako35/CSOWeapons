...
public plugin_init(){
	...
	thundergun = zp_register_extra_item("Thunder Ghost Walker", 0, ZP_TEAM_HUMAN|ZP_TEAM_SURVIVOR);
}
...
public UTIL_Explode_1(id){
	...
	while((victim = engfunc(EngFunc_FindEntityInSphere, victim, fStart, 100.0)) != 0){
		...
		ExecuteHamB(Ham_TakeDamage, victim, 0, id, get_pcvar_float(cvar_radius_damage), DMG_SHOCK|DMG_NEVERGIB);
...
public UTIL_Explode_2(id){
	...
	victim = -1
	while((victim = engfunc(EngFunc_FindEntityInSphere, victim, fStart, 100.0)) != 0){
		new dmg = get_pcvar_float(cvar_radius_damage)
		if(!is_user_alive(victim) || !zp_get_user_zombie(victim)){
			continue
		}
		set_msg_block(g_death, BLOCK_SET);
		ExecuteHamB(Ham_TakeDamage, victim, 0, id, dmg *= 3.0, DMG_SHOCK|DMG_NEVERGIB);
		set_msg_block(g_death, BLOCK_NOT);
		if(get_user_health(victim) <= 0){
