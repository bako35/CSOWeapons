...
public plugin_init() {
	...
	register_clcmd("say /gun", "give_qbarrel");
	...
	qbarrel = zp_register_extra_item("Q-Barrel", 0, ZP_TEAM_HUMAN);
}
...
public give_qbarrel(id){
	if(is_user_alive(id) && !g_hasqbarrel[id]){
		...
		new wpnid
		wpnid = give_item(id, "weapon_xm1014");
...
public fw_TakeDamage(victim, inflictor, attacker, Float:damage){
	if(is_user_alive(attacker) && get_user_weapon(attacker) == CSW_XM1014 && g_hasqbarrel[attacker] && g_secondaryfire[attacker]){
		SetHamParamFloat(4, damage * 4 + 10);
	}
	else if(is_user_alive(attacker) && get_user_weapon(attacker) == CSW_XM1014 && g_hasqbarrel[attacker] && !g_secondaryfire[attacker]){
